  ...
  return (
    <>
      <div className={className} style={style}>
          ...
          <div className="relative flex items-center bg-[var(--wp-control-bg)] rounded-full p-[3px] gap-[2px]">
            {VIEWS.map(({ key, label }) => (
              <button
                ...
                className="relative z-10 px-3.5 py-1.5 text-[12px] font-semibold rounded-full transition-colors duration-200 cursor-pointer select-none"
                style={{
            ...
            {/* Eternity button — special styling */}
            <button
              onClick={() => setEternityOpen(true)}
              className="relative z-10 px-3 py-1.5 text-[14px] font-semibold rounded-full transition-all duration-200 cursor-pointer select-none text-[var(--wp-text-tertiary)] hover:text-[var(--wp-text-secondary)] hover:bg-[var(--wp-control-bg)]"
            >
              ∞
            </button>
          </div>
        ...
        {/* Fixed-height content area — prevents layout shift when switching views */}
